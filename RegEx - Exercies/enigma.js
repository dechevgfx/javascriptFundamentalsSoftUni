function finalFunction(input) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (newLine.match(pattern)) {
      let planet = newLine.match(/@[A-Za-z]+/g)
      const population = newLine.match(/:[0-9]+/g)
      const attackOrDestruction = newLine.match(/![AD]+!/g)
      const soldierCount = newLine.match(/->[0-9]+/g)
